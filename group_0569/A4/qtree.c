...
int find_user(QNode *current, char *name, int *answer, int i){
    // current is an inner node
    if (current->node_type == REGULAR){
        int exist;
        // look for the node (recursively) in the 0 subtree
        answer[i] = 0;
        exist = find_user(current->children[0].qchild, name, answer, i+1);
        if (exist == 1)
            return 1;
            
        // look for the node (recursively) in the 1 subtree
        answer[i] = 1;
        exist = find_user(current->children[1].qchild, name, answer, i+1);
        if (exist == 1)
            return 1;
